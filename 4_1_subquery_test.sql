#문1) 2010년 이후에 입사한 남자 중 급여를 가장 많이 받는 직원은?[이후 : 기준년  포함]
...
#문3) '이미라' 직원의 입사 이후에 입사한 직원은? [이후 : 기준년  포함]
...
SELECT jikwonname, busernum, jikwonibsail ,jikwonpay  FROM jikwon
	WHERE jikwonibsail BETWEEN '2010-01-01' AND '2015-12-31'
	AND not busernum = 40 AND jikwonpay 
	IN(SELECT MIN(jikwonpay) FROM jikwon 
		WHERE jikwonibsail BETWEEN '2010-01-01' AND '2015-12-31' 
...
/* 
[DISTINCT 사용!]
왜 LEFT OUTER JOIN 일까? - 부서명(busername) 가져오기, 부서 없어도 직원은 보여라
	inner join을 쓰면 부서 없는 직원은 사라짐
*/
...
SELECT jikwonname AS 직원명, busername AS 부서명, busertel AS 부서전화,
	...
	case 
	when TIMESTAMPDIFF(YEAR,(SUBSTR(gogekjumin,1,6)),NOW()) <= 30 THEN '청년'
	...
	ORDER BY TIMESTAMPDIFF(YEAR,(SUBSTR(gogekjumin,1,6)),NOW()) DESC;